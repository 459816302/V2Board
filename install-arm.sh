...
install_XrayR() {
    ...
    if  [ $# == 0 ] ;then
        last_version=$(curl -Ls "https://raw.githubusercontent.com/expfukck/XrayR-V2Board/main/api" | grep '"tag_name":' | sed -E 's/.*"([^"]+)".*/\1/')
        if [[ ! -n "$last_version" ]]; then
            echo -e "${red}检测 XrayR 版本失败，可能是超出 Github API 限制，请稍后再试，或手动指定 XrayR 版本安装${plain}"
            exit 1
        fi
        echo -e "检测到 XrayR 最新版本：${last_version}，开始安装"
        wget -N --no-check-certificate -O /usr/local/XrayR/XrayR-linux-64.zip https://github.com/expfukck/YXrayR/releases/download/${last_version}/XrayR-linux-arm64-v8a.zip
        if [[ $? -ne 0 ]]; then
        ...
        last_version=$1
        url="https://github.com/expfukck/YXrayR/releases/download/${last_version}/XrayR-linux-64.zip"
    ...
    file="https://github.com/expfukck/XrayR-V2Board/raw/main/XrayR.service"
    ...
    if [[ ! -f /etc/XrayR/config.yml ]]; then
        ...
        echo -e ""
        echo -e "全新安装，请先参看教程：https://github.com/expfukck/XrayR-V2board，配置必要的内容"
    else
        ...
        if [[ $? == 0 ]]; then
            ...
            echo -e "${red}XrayR 可能启动失败，请稍后使用 XrayR log 查看日志信息，若无法启动，则可能更改了配置格式，请前往 wiki 查看：https://github.com/expfukck/YXrayR/wiki${plain}"
        fi
    fi
    ...
    curl -o /usr/bin/XrayR -Ls https://raw.githubusercontent.com/expfukck/XrayR-V2Board/master/XrayR.sh
    ...
    echo
    #curl -o /usr/bin/XrayR-tool -Ls https://raw.githubusercontent.com/expfukck/YXrayR/master/XrayR-tool
    ...
    echo "Telegram: https://t.me/expfukck"
    echo "Github: https://github.com/expfukck/XrayR-V2Board"
